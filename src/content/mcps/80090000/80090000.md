---
uid: "80090000"
introduction: "基于MCP协议的专业金融数据服务器，集成Tushare API，为AI助手提供实时金融数据和技术指标分析。"
---
...
[![](https://smithery.ai/badge/@guangxiangdebizi/FinanceMCP)](https://smithery.ai/server/@guangxiangdebizi/FinanceMCP)
...
![image](https://gips3.baidu.com/it/u=146767623,710663135&fm=3081&app=3081&f=PNG?w=1920&h=1080)
...
```
1用户请求 → 参数解析 → 数据需求计算 → 扩展历史数据获取 → 指标计算 → 结果返回
```
...
1
2
3"分析茅台(600519.SH)技术面状况，计算MACD(12,26,9)、RSI(14)、KDJ(9,3,3)"
...
**🧱 CSI 指数成分与权重（含估值/财务）**
```
1"获取中证证券公司(399975.SZ) 在 2024-01-01 至 2024-06-30 区间的成分股摘要（含PE、PB、股息率、ROE、ROA、净利率、经营现金流、资产负债率、营收同比、资产周转率、毛利率、三费比率、现金分红率）"
```
...
1
2
3"获取腾讯控股(00700.HK) 2024年利润表，包含关键财务比率"
...
2
3
4"分析英伟达(NVDA) 2024年财务表现，包括利润表和现金流"
...
1
2
3"查看比特币(BTC-USD) 2024-01-01 至 2024-06-30 的走势，计算 MACD(12,26,9) 和 RSI(14)"
...
BASH```
1npx -y @smithery/cli install @guangxiangdebizi/FinanceMCP --client claude
```
...
**支持公司**：所有港交所上市公司，包括腾讯([00700.HK](http://00700.HK))、阿里巴巴([09988.HK](http://09988.HK))、建设银行([00939.HK](http://00939.HK))等。
